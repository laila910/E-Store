...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (count($errorMessages) == 0) {
        ...
        $sql = "SELECT `product`.`productname`,`productdetails`.* FROM `product` join `productdetails` on `product`.`id` =`productdetails`.`product_Id` where `product`.`productname` like '%$key%' or `productdetails`.`product_Description` like '%$key%' or `productdetails`.`product_Specificaton`  like '%$key%'";
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                ...
                if (isset($_SESSION['data'])) {
                    ...
                    while ($data = mysqli_fetch_array($op)) {
